...
// AuthContextPayload is now defined in UserModels.swift
...
struct StrategicTestView: View {
    ...
    // Add FFI String Manager for safer memory management
    private let ffiManager = StrategicGoalFFIHandler()
    ...
    // MARK: - Private Structs for Complex Initializers
    // These structs help the Swift compiler by providing explicit types,
    // resolving the "unable to type-check in reasonable time" error.
    
    private struct DocumentTypeSetupInfo {
        let name: String
        let extensions: String
        let maxSize: Int
        let compressionLevel: Int
        let compressionMethod: String
        let minSizeForCompression: Int
        let priority: String
    }
    
    private struct DocumentUploadInfo {
        let name: String
        let ext: String
        let typeId: String
        let title: String
    ...
    func setupDocumentTypes() async {
        ...
        let documentTypes: [DocumentTypeSetupInfo] = [
            .init(name: "Strategic Plan", extensions: "pdf,doc,docx", maxSize: 10485760, compressionLevel: 7, compressionMethod: "lossless", minSizeForCompression: 102400, priority: "high"),
            .init(name: "Photo Evidence", extensions: "jpg,jpeg,png,heic", maxSize: 5242880, compressionLevel: 8, compressionMethod: "lossy", minSizeForCompression: 51200, priority: "normal"),
            .init(name: "General Report", extensions: "pdf,xlsx,csv,docx,txt", maxSize: 10485760, compressionLevel: 6, compressionMethod: "lossless", minSizeForCompression: 102400, priority: "normal")
        ]
    ...
    func uploadDocumentsWithPriorities() async {
        guard let goalId = adminGoalId,
              let spDocId = strategicPlanDocTypeId,
              let grDocId = generalReportDocTypeId,
              let peDocId = photoEvidenceDocTypeId else {
            updateResults("❌ Cannot upload documents: Missing Goal ID or Document Type IDs.")
            return
        }
        ...
        let documentsToUpload: [DocumentUploadInfo] = [
            .init(name: "test_strategic_plan", ext: "pdf", typeId: spDocId,
                  title: "Q4 Strategic Plan", linkedField: "actual_value", priority: "High", compressionPriority: "High"),
            .init(name: "test_report", ext: "pdf", typeId: grDocId,
                  title: "Monthly Report", linkedField: "target_value", priority: "NORMAL", compressionPriority: "NORMAL"),
            .init(name: "test_image", ext: "jpg", typeId: peDocId,
    ...
    private func loadTestDocument(named name: String, type ext: String) -> Data? {
        ...
        // If file doesn't exist in bundle, create mock data
        updateResults("⚠️ \(name).\(ext) not found in bundle, creating mock data")
        return createMockDocumentData(for: ext, size: getMockSize(for: name))
    }
    
    private func createMockDocumentData(for fileExtension: String, size: Int) -> Data {
...
"""
        ...
        case "jpg", "jpeg":
            ...
            while data.count < size {
                data.append(contentsOf: Array(0...255).map { UInt8($0 % 256) })
            }
            // Add JPEG end marker
        ...
        default:
            // Generic text file
            let content = "Mock \(fileExtension.uppercased()) document content for testing compression.\n"
            var data = Data()
            while data.count < size {
    ...
    private func getMockSize(for filename: String) -> Int {
        switch filename {
        case "test_strategic_plan":
            return 3_200_000 // ~3.2MB
        case "test_report":
            return 1_500_000 // ~1.5MB  
        case "test_image":
            return 6_800_000 // ~6.8MB
        default:
    ...
    private func unregisterDocumentUsage(_ documentId: String) async {
        let payload = """
        {
            ...
            "user_id": "\(self.authState.lastLoggedInUser?.userId ?? "00000000-0000-0000-0000-000000000000")",
    ...
    private func getAuthContextAsJSONString(roleOverride: String? = nil) -> String {
        let authData = AuthContextPayload(
            user_id: self.authState.lastLoggedInUser?.userId ?? "00000000-0000-0000-0000-000000000000",
            role: roleOverride ?? self.authState.lastLoggedInUser?.role ?? "admin",
            device_id: AuthenticationState.getDeviceId(),
            offline_mode: false
        )
    ...
    private func getAuthContext() -> AuthContextPayload {
        return AuthContextPayload(
            user_id: self.authState.lastLoggedInUser?.userId ?? "00000000-0000-0000-0000-000000000000",
            role: self.authState.lastLoggedInUser?.role ?? "admin",
            device_id: AuthenticationState.getDeviceId(),
            offline_mode: false
        )
    }
...
// MARK: - Preview Provider (MODIFIED)
// This creates a self-contained, authenticated preview for StrategicTestView.
#if DEBUG

/// A helper view that sets up the environment asynchronously before showing the real preview.
/// This is crucial for allowing the Rust core's async runtime to initialize correctly.
struct PreviewLoadingView: View {
    @State private var authState: AuthenticationState? = nil

    var body: some View {
        if let authState = authState {
            // Once the authState is populated, show the actual view.
            NavigationView {
                StrategicTestView()
            }
            .environmentObject(authState)
        } else {
            // While setting up, show a progress indicator.
            ProgressView("Setting up preview...")
                .task {
                    // Perform the async setup when the view appears.
                    self.authState = await PreviewHelper.setupPreviewEnvironmentAsync()
                }
        }
    }
}
...
struct StrategicTestView_Previews: PreviewProvider {
    static var previews: some View {
        // We now use the PreviewLoadingView to ensure safe, async initialization.
        PreviewLoadingView()
    }
}
#endif
...
// MARK: - FFI Function Declarations have been moved to PreviewHelper.swift for better project structure.
