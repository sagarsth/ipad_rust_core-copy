    ...
    DeleteServiceRepository<Workshop> + MergeableEntityRepository<Workshop> + Send + Sync
{
    ...
    /// Find workshops by specific IDs
...
impl WorkshopRepository for SqliteWorkshopRepository {
    ...
    async fn find_by_ids(
        ...
        let count_query = format!(
            "SELECT COUNT(*) FROM workshops WHERE id IN ({}) AND deleted_at IS NULL",
            count_placeholders
        );
        ...
        let select_query = format!(
            "SELECT * FROM workshops WHERE id IN ({}) AND deleted_at IS NULL ORDER BY event_date DESC, created_at DESC LIMIT ? OFFSET ?",
            select_placeholders
        );
