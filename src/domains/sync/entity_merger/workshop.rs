...
/// Entity merger for `workshops` table – parallels DonorEntityMerger.
pub struct WorkshopEntityMerger {
    repo: Arc<dyn WorkshopRepository + Send + Sync>,
    pool: SqlitePool,
    delete_service: Arc<dyn DeleteService<Workshop> + Send + Sync>,
}

impl WorkshopEntityMerger {
    pub fn new(
        repo: Arc<dyn WorkshopRepository + Send + Sync>,
        pool: SqlitePool,
        delete_service: Arc<dyn DeleteService<Workshop> + Send + Sync>,
...
#[async_trait]
impl DomainEntityMerger for WorkshopEntityMerger {
    fn entity_table(&self) -> &'static str { "workshops" }
