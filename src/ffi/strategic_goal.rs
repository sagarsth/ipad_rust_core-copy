...
// ============================================================================
// STRATEGIC GOAL DOMAIN – FFI BINDINGS (CRUD ONLY)
// ============================================================================
// This file exposes a minimal set of `StrategicGoalService` operations across
// the C ABI so that Swift can create, read, update, list and delete goals.  The
// wrapper patterns are identical to those in `ffi/user.rs` – feel free to copy
// them when you need to surface additional endpoints.
// ----------------------------------------------------------------------------
...
use crate::domains::strategic_goal::types::{NewStrategicGoal, UpdateStrategicGoal};
...
/// Payload { "new_goal": NewStrategicGoal, "auth": AuthCtxDto }
#[unsafe(no_mangle)]
pub unsafe extern "C" fn strategic_goal_create(payload_json: *const c_char, result: *mut *mut c_char) -> c_int {
    handle_status_result(|| unsafe {
        ...
        #[derive(Deserialize)] struct P { new_goal: NewStrategicGoal, auth: AuthCtxDto }
        ...
        let resp = block_on_async(svc.create_strategic_goal(p.new_goal, &auth)).map_err(FFIError::from_service_error)?;
...
pub unsafe extern "C" fn strategic_goal_get(payload_json: *const c_char, result: *mut *mut c_char) -> c_int {
    handle_status_result(|| unsafe {
        ...
        let auth: AuthContext = p.auth.try_into()?;
        let svc = globals::get_strategic_goal_service()?;
        let resp = block_on_async(svc.get_strategic_goal_by_id(id, None, &auth)).map_err(FFIError::from_service_error)?;
        *result = CString::new(serde_json::to_string(&resp).unwrap()).unwrap().into_raw();
        Ok(())
    })
}
...
pub unsafe extern "C" fn strategic_goal_list(payload_json: *const c_char, result: *mut *mut c_char) -> c_int {
    handle_status_result(|| unsafe {
        ...
        let auth: AuthContext = p.auth.try_into()?;
        let svc = globals::get_strategic_goal_service()?;
        let paged = block_on_async(svc.list_strategic_goals(params, None, &auth)).map_err(FFIError::from_service_error)?;
        *result = CString::new(serde_json::to_string(&paged).unwrap()).unwrap().into_raw();
        Ok(())
    })
}

/// Payload { "id": "uuid", "update": UpdateStrategicGoal, "auth": AuthCtxDto }
#[unsafe(no_mangle)]
pub unsafe extern "C" fn strategic_goal_update(payload_json: *const c_char, result: *mut *mut c_char) -> c_int {
    handle_status_result(|| unsafe {
        ensure_ptr!(payload_json); ensure_ptr!(result);
        let s = CStr::from_ptr(payload_json).to_str().map_err(|_| FFIError::invalid_argument("utf8"))?;
        #[derive(Deserialize)] struct P { id: String, update: UpdateStrategicGoal, auth: AuthCtxDto }
        let p: P = serde_json::from_str(s).map_err(|e| FFIError::invalid_argument(&format!("json {e}")))?;
        let id = Uuid::parse_str(&p.id).map_err(|_| FFIError::invalid_argument("uuid"))?;
        let auth: AuthContext = p.auth.try_into()?;
        let svc = globals::get_strategic_goal_service()?;
        let resp = block_on_async(svc.update_strategic_goal(id, p.update, &auth)).map_err(FFIError::from_service_error)?;
        *result = CString::new(serde_json::to_string(&resp).unwrap()).unwrap().into_raw();
        Ok(())
    })
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn strategic_goal_delete(payload_json: *const c_char) -> c_int {
    handle_status_result(|| unsafe {
        ...
        let auth: AuthContext = p.auth.try_into()?;
        let svc = globals::get_strategic_goal_service()?;
        block_on_async(svc.delete_strategic_goal(id, p.hard_delete.unwrap_or(false), &auth)).map_err(FFIError::from_service_error)?;
        Ok(())
    })
}
